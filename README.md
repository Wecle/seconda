...
Agent 面试房间的回归步骤见 [UX 验证清单](docs/operations/agent-room-ux-checklist.md)。
...
Agent 面试是新建会话与所有 active 历史会话的唯一执行路径，不使用版本或环境变量切换执行器：
...
`reasoning_delta` 与 `response_delta` 是可重放的公开 provisional 内容；只有收到 `message_committed` 后问题才是正式消息。一旦 provisional 内容已展示，该 Run 不会静默切换模型或把另一个 attempt 的文本拼接到同一消息。
...
迁移完成后，legacy 新建、回答、下一题和完成接口统一返回 `410 Gone`。已完成的历史会话、问题、评分、报告和分享链接继续可读；尚未完成的历史会话由 cutover 命令迁移到最新 Agent Runtime 后继续执行。
