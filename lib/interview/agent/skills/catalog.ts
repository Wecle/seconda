...
const skills: InterviewSkill[] = [
  ...
  },
  {
    ...
    instructions: "基于最新回答和已注入覆盖度，在 submit_interview_turn 中同时提交无分数轻量评估、当前回答分类的主题变化和一个追问、新主题或结束行动。coverageChanges 只提交当前回答分类；最终状态由服务端根据 assessment.followUpNeeded 和该分类题数确定，其他分类变化会被忽略。不得生成正式分数。responseText 必须围绕本轮唯一的结构化核心考察意图，可以包含必要的回答提示，但不得切换到无关主题；评价只复述已确认内容，不做人格判断。",
